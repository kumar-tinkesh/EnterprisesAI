"""Pydantic v2 request/response contracts for the Vendor domain API.

Covers MCP server registration/lifecycle, OAuth bootstrap, and tenant
grants — everything gated behind ``vendor_admin`` (plus the public OAuth
callback). Read-only catalog/search/plan-tool-call contracts consumed by
regular users live in ``user.api.v1.schemas`` instead.
"""
