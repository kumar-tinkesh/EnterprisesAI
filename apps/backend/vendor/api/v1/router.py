...
code. The router is mounted by ``apps.backend.main`` (and ``apps.auth.src.main``)
under ``/api/v1/vendor/resources``, alongside ``user.api.v1.router`` under the
same prefix (see those modules for why — point 2 of the vendor/user split is
...
logger = logging.getLogger("vendor.router")
...
# consent screen). See vendor.services.oauth_flow for how this
