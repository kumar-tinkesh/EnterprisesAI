"""Shared pytest fixtures for the ``vendor`` and ``user`` domain tests.

Lives at ``apps/backend/`` (the common parent of ``vendor/tests`` and
``user/tests``) so both domains' test suites see the same isolated test
database and API-client fixtures without duplicating this setup in each
domain — the two ``tests/`` packages only hold their own domain-specific
tests, no fixture duplication.
...
def _mock_mcp_network(monkeypatch):
    ...
    the ``mcp_service`` and ``vendor.api.v1.router`` call sites so CRUD tests
    are fast and deterministic.
    """
    import importlib

    _router_mod = importlib.import_module("vendor.api.v1.router")
    _svc_mod = importlib.import_module("vendor.services.mcp_service")
